...
df_both <- df_both %>% mutate(len_abs = nchar(abstract), pct = ntile(len_abs, 100)) %>% 
  ...
  # Here: 34,868
  # aqui drop los 9
# Drop articles with information
df_both <- df_both %>% filter(info == 1, !is.na(all_category)) # 9
  # Here: 31,171
...
#ggplot(preds_point2, aes(x = proportion, y = reorder(topic, fem_tot), color = value)) + geom_text(aes(label=labs), size = 4) + scale_color_brewer(palette = "PRGn")  + geom_line()
...
ggplot(preds_point3) +
  ...
  facet_wrap( ~ bigg_fem, ncol = 1, scales = "free_y", labeller = as_labeller(facet_names)) #+ scale_x_discrete(position = "top") 
